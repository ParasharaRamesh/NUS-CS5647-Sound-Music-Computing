...
def main(best_model_path = "best_model.pth"):
    args = {
        'dataset_root': '.\\data_mini\\',
        'test_dataset_path': '.\\data_mini\\test',
        'output_dir': '.\\results',
    ...
    best_model_path = os.path.join(args['save_model_dir'], best_model_path)
    ...
    #results contains the results for 5 songs ( 91 -> 95)
    save_json(results, args['output_path'], sort=True)

    #converting to midi output
    # for song in range(91,96):
    #     convert_to_midi(results, str(song), jpath(args['output_dir'], f'{song}_out.mid'))
